...
//
// Adapted from projects/vitrina-site/tests/support/fakedb.mjs. Mug's schema
// uses more validator forms (v.id, v.literal, v.array, v.object) and named
// validators from convex/lib/listing.ts, so the parser here knows those too.
//
...
const NAMED = new URL('../../convex/lib/listing.ts', import.meta.url);
...
let NAMED_VALIDATORS = {};

function parseValidator(expr) {
  // A multi-line call can end with a trailing comma inside its parentheses.
  const text = expr.trim().replace(/,\s*$/, '').trim();
  if (/^[A-Za-z_]\w*$/.test(text)) {
    const named = NAMED_VALIDATORS[text];
    if (!named) throw new Error(`fakedb: unknown named validator ${text}; export it from convex/lib/listing.ts`);
    return named;
  }
  if (text === 'v.string()') return (x) => typeof x === 'string';
  if (text === 'v.number()' || text === 'v.float64()') return (x) => typeof x === 'number';
  let lit = text.match(/^v\.literal\(\s*"([^"]*)"\s*\)$/);
  if (lit) return (x) => x === lit[1];
  let idm = text.match(/^v\.id\(\s*"(\w+)"\s*\)$/);
  if (idm) {
    const table = idm[1];
    return (x) => typeof x === 'string' && x.startsWith(`${table}:`);
  }
  let arr = text.match(/^v\.array\(([\s\S]*)\)$/);
  if (arr) {
    const inner = parseValidator(arr[1]);
    return (x) => Array.isArray(x) && x.every((item) => inner(item));
  }
  let obj = text.match(/^v\.object\(\s*\{([\s\S]*)\}\s*\)$/);
  if (obj) {
    const fields = {};
    for (const part of splitTop(obj[1])) {
      const colon = part.indexOf(':');
      fields[part.slice(0, colon).trim()] = parseValidator(part.slice(colon + 1));
    }
    return (x) => {
      if (x === null || typeof x !== 'object' || Array.isArray(x)) return false;
      for (const key of Object.keys(x)) if (!fields[key] && x[key] !== undefined) return false;
      return Object.entries(fields).every(([key, check]) => check(x[key]));
    };
  }
...
// `export const name = v....;` blocks from convex/lib/listing.ts, in file
// order, so a later validator can use an earlier one by name.
export function parseNamed(source) {
  const text = source.replace(/\/\*[\s\S]*?\*\//g, '').replace(/\/\/[^\n]*/g, '');
  const named = {};
  for (const match of text.matchAll(/export const (\w+)\s*=\s*(v\.)/g)) {
    const start = match.index + match[0].length - 2;
    const open = text.indexOf('(', start);
    const body = balanced(text, open);
    NAMED_VALIDATORS = named;
    named[match[1]] = parseValidator(text.slice(start, open) + '(' + body + ')');
  }
  return named;
}

export function parseSchema(source, namedSource) {
  NAMED_VALIDATORS = parseNamed(namedSource ?? readFileSync(NAMED, 'utf8'));
  const text = source.replace(/\/\*[\s\S]*?\*\//g, '').replace(/\/\/[^\n]*/g, '');
  const tables = {};
